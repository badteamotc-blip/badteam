...
# ── Конфигурация ─────────────────────────────────────────────────────────────
BOT_TOKEN = '8835938032:AAEiEKNW0tCHmHskCczAvQCaYk4Xxo_6YVA' 
OWNERS = [8794223703]
TEAM_NAME = 'bad team'
...
OTS_INSTANCES_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ots_instances')
OTS_SOURCE_DIR    = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'отс')
...
async def cb_info(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        reply_markup=kb(
            [InlineKeyboardButton.model_construct(text='Чат команды', url='https://t.me/bad_team_ton', icon_custom_emoji_id='5443038326535759644')],
            [btn('Назад', 'back_main', 5960671702059848143)]))
    await cb.answer()
